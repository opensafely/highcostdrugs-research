...
select
	...
	sum(case
			when DispensingRoute is NULL then 1
			when DispensingRoute = 'NULL' then 1
			else 0
		end) as NULL_Records,
	sum(case	
			when DispensingRoute not like '%[^0-9]%' then 1
			else 0
		end) as Numeric_Records,
	count(distinct(DispensingRoute)) as Unique_Values
